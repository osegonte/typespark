...
if [ -z "$PYTHON_CMD" ]; then
  echo "⚠️ Python not found in PATH. Checking for local virtual environment..."
  
  # Check for virtual environment
  if [ -d "backend/venv" ] && [ -f "backend/venv/bin/python" ]; then
    PYTHON_CMD="backend/venv/bin/python"
    echo "✅ Found Python in virtual environment: $PYTHON_CMD"
    ...
    echo "❌ Python not found. The script will continue but may not be able to run Python commands."
    echo "   You may need to manually activate your virtual environment first."
  fi
fi

# Check if npm is available
if ! command_exists npm; then
  echo "⚠️ Node.js/npm not found. The script will continue but may not be able to run npm commands."
fi
...
if [ -f "backend/app.py" ]; then
  ...
  if ! grep -q "from flask_cors import CORS" backend/app.py; then
    # Add CORS import if not present
    sed -i.bak '1i\from flask_cors import CORS' backend/app.py
    echo "✅ Added CORS import to app.py"
  fi

  # Check if CORS is already configured
  if ! grep -q "CORS(app" backend/app.py; then
    # Add CORS configuration after app definition
    sed -i.bak '/app = Flask/a\
\
...
CORS(app, \
    ...
    echo "✅ Added CORS configuration to app.py"
  else
    echo "✅ CORS already configured in app.py"
  fi
...
if [ -f "frontend/src/pages/HomePage.js" ]; then
  ...
  if ! grep -q "<ConnectionTester />" frontend/src/pages/HomePage.js; then
    # Add ConnectionTester component at the beginning of the return statement
    # This is a bit complex with sed, so we'll do a more robust approach
    TEMP_FILE=$(mktemp)
    awk '
    /return[[:space:]]*\(/ { 
      print $0;
      print "      <ConnectionTester />";
      next;
    }
    { print }' frontend/src/pages/HomePage.js > "$TEMP_FILE"
    cat "$TEMP_FILE" > frontend/src/pages/HomePage.js
    rm "$TEMP_FILE"
...
# 7. Update API URLs in DebugInfo.js
echo "7. Updating DebugInfo.js..."
if [ -f "frontend/src/DebugInfo.js" ]; then
  if grep -q "localhost:5001/api/health" frontend/src/DebugInfo.js; then
    # Replace port 5001 with 5002
    sed -i.bak 's/localhost:5001\/api\/health/localhost:5002\/api\/health/g' frontend/src/DebugInfo.js
    echo "✅ Updated API URL in DebugInfo.js"
  elif grep -q "localhost:5002/api/health" frontend/src/DebugInfo.js; then
    echo "✅ API URL already using port 5002 in DebugInfo.js"
  else
    echo "⚠️ API URL pattern not found in DebugInfo.js"
  fi
else
  echo "⚠️ frontend/src/DebugInfo.js not found, skipping"
fi
...
if [ -f "run.sh" ]; then
  ...
  sed -i.bak 's/localhost:5001/localhost:5002/g' run.sh
...
echo "Starting the backend with Python: $PYTHON_CMD"
cd backend
if [ -d "venv" ] && [ -f "venv/bin/activate" ]; then
  echo "Activating backend virtual environment..."
  source venv/bin/activate
fi
$PYTHON_CMD app.py --port=5002 &
BACKEND_PID=$!
cd ..
...
# Create a manual run script for people having issues with the automatic one
echo "10. Creating manual run instructions..."
cat > run-manual.txt << 'EOL'
MANUAL STARTUP INSTRUCTIONS

If you're having issues with the automatic run.sh script, follow these steps:
...
1. Start the backend:
   ...
   source venv/bin/activate  # or appropriate command for your system
...
3. Access the application:
   - Backend API should be available at http://localhost:5002
   - Frontend should be available at http://localhost:3000
   
If you need to debug connection issues:
- Check browser console (F12) for frontend errors
- Check backend terminal for any error messages
- The ConnectionTester component will help diagnose issues
EOL
echo "✅ Created manual run instructions"
...
echo "   ./run.sh"
echo
echo "If that doesn't work, follow the manual instructions in run-manual.txt"
echo "  cat run-manual.txt"
echo
